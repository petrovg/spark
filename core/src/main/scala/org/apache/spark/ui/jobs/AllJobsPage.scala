...
private[ui] class AllJobsPage(parent: JobsTab, store: AppStatusStore) extends WebUIPage("") {
  ...
  private def jobsTable(
    ...
    val paramsJava = request.getParameterMap.asInstanceOf[java.util.Map[String, Array[String]]]
    val params: Map[String, Array[String]] = paramsJava.asScala.toMap
    val allParameters = params.mapValues(_.map(UIUtils.stripXSS))
